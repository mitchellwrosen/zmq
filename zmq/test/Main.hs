...
import Zmq.XPublisher qualified as XPub
import Zmq.XSubscriber qualified as XSub
  ...
  for_ sockets \SomeSocket {name, with, close, bind, unbind, connect, disconnect} -> do
    describe name do
      ...
      it "unbind bogus" \ctx ->
        with ctx \sock ->
          unbind sock (Zmq.Tcp "")
            `shouldThrow` (== Zmq.Error "zmq_unbind" Zmq.EINVAL "Invalid argument")
      ...
      it "disconnect bogus" \ctx ->
        with ctx \sock ->
          disconnect sock (Zmq.Tcp "")
            `shouldThrow` (== Zmq.Error "zmq_disconnect" Zmq.EINVAL "Invalid argument")
